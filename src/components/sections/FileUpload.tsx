...
const FileUpload: React.FC<FileUploadProps> = ({ selectedFile, onFileSelect, disabled }) => {
  ...
  if (selectedFile) {
    return (
      <div className="mt-4 p-4 bg-gray-700/50 border border-gray-600 rounded-lg flex items-center justify-between">
        <p className="text-sm text-cyan-300 font-medium truncate pr-4">{selectedFile.name}</p>
        <button onClick={resetFile} disabled={disabled} className="text-gray-400 hover:text-white transition-colors disabled:opacity-50">
    ...
    <div className="mt-4">
      ...
      >
        <div className="flex flex-col items-center justify-center pt-5 pb-6">
           ...
           <p className="text-xs text-gray-500">PDF, DOCX, PPTX, XLSX, TXT, MD files supported</p>
        </div>
        <input 
          id="file-upload" 
          type="file" 
